...
class _LobbyScreenState extends State<LobbyScreen> {
        ...
        SafeArea(
          child: Padding(
            ...
            child: Row(
              ...
              children: <Widget>[
                const Text("WIND", style: TextStyle(fontSize: 32,fontWeight: FontWeight.bold),),
                Row(
                  children: <Widget>[
                    InkWell(
                      onTap: () => Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (BuildContext context) => MutualRequestScreen(
                              provider: provider, mutualRequests: mutualRequests),
                        ),
                      ),
                      child: Container(
                        ...
                        child: Row(
                          children: const <Widget>[
                            ...
                            SizedBox(width: 2,),
                            Text("Add User",style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                          ],
                        ),
                      ),
                    ),
        ...
        StreamBuilder<WINDWebSocketResponse>(
          ...
          builder: (context, snapshot) {
            ...
            return mutualUsers.isNotEmpty
                          ...
                          children: [
                            SlidableAction(
                              onPressed: (context) {
                                // provider.
                                setState(() {
                                  provider.WINDMutualUserDeleter(name: mutualUsers[index].name);
                ...
                : Container(child: const Text("No Friends.."));
          },
        ),
        // Row(
  ...
  void WINDResponseMutualUsers({required dynamic payload}) {
    mutualUsers = [];
    print(payload);
  ...
  addListMutualUsers({required String id, required String name}) async {
    int userId = int.parse(await StorageManager.readData('user_id'));
    String _chatId = (int.parse(id) ^ userId).toString();
    String _datetime = base64.encode(utf8.encode(DateFormat('yyyy-M-d HH:mm:ss.S000', 'ko_KR').format(DateTime.now().toUtc().add(const Duration(hours: 9)))));
    String _lastMessage = "";
    String _lastTime = "";
    provider.WINDLoadChatContents(chatID: _chatId, datetime: _datetime, count: 1);
    // final subscription = provider.windStream.listen(null);
    // subscription.onData((event) async {
    //   _lastMessage = await event.payload['content'];
    mutualUsers.add(MutualList(provider: provider, id: id, chatId: _chatId, name: name, messageText: _lastMessage, time: _lastTime));
    //   subscription.cancel();
    // });
